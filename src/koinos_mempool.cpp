...
using timer_func_type = std::function<
  void( const boost::system::error_code&, std::shared_ptr< koinos::mempool::mempool >, std::chrono::seconds ) >;
...
int main( int argc, char** argv )
{
  ...
  try
  {
    ...
    std::shared_ptr< koinos::mempool::mempool > mempool =
      std::make_shared< koinos::mempool::mempool >( fork_algorithm );
...
#pragma clang diagnostic pop
      ...
      [ & ]( const std::string& msg )
      {
        ...
        try
        {
          ...
          *pending_transaction.mutable_transaction() = trx_accept.transaction();
          pending_transaction.set_timestamp( std::chrono::duration_cast< std::chrono::milliseconds >(
          ...
          auto rc_used = mempool->add_pending_transaction( pending_transaction, trx_accept.receipt().max_payer_rc() );
    ...
    request_handler.add_broadcast_handler( "koinos.transaction.fail",
                                           [ & ]( const std::string& msg )
                                           {
